% [ 자른 파형 생성 ] : Cutting + Y축 이동 + 빈배열 없음 
function z = read_wave5(FN1,FN2,num) % C1--new , C2-new , 개수 
...
for i= st: st + num - 1 
    ...
    trace = read_lecroywave("./sigmoid/"+FN1+"--"+index+".trc",i-1);  %%% 파일명 주의 
    trigger = read_lecroywave("./sigmoid/"+FN2+"--"+index+".trc",i-1);  %%% 파일명 주의 
    ...
    if count > 1
        if length(z(:,k-1)) > len 
            z(:,count) = vertcat(trace.y(start:end_) - 3.2 ,(zeros(length(z(:,count-1)) - len , 1)));
            
            %fprintf("%d",length(z(:,k-1)));
        else
            z(len,count) = 0;
            z(:,count) = trace.y(start:end_) - 3.2;
