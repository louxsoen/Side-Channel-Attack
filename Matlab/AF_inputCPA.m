...
function corr = AF_inputCPA(FN,trace) % 파일 이름 넘겨주기 ( "tanh_input" / tanh_trace ) 
...
for i=1 : trNum
    ...
    input(:,i) = fread(inputFile, 1,'uint32'); % input(1) 
    
    %fprintf("%x \n" ,input(1,i)); % 입력값 확인 
    HW(1,i) = sum(dec2bin(input(1,i)).' == '1'); % 활성함수 입력값의 헤밍웨이트
end
