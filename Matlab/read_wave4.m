% [ 자른 파형 리턴 ] Y축 옮기기
function z = read_wave4(FN1,FN2,num) % C1--new , C2-new , 개수 
...
for i= st: st + num - 1
    ...
    trace = read_lecroywave("./wave/"+FN1+"--"+index+".trc",i-1);  %%% 파일명 주의 
    trigger = read_lecroywave("./wave/"+FN2+"--"+index+".trc",i-1);  %%% 파일명 주의 
    ...
    if count > 0
        if length(z(:,i-1)) > len 
            z(:,i) = vertcat(trace.y(start:end_) - 3.2 ,(zeros(length(z(:,i-1)) - len , 1)));
            
            %fprintf("%d",length(z(:,i-1)));
        else
            z(len,i) = 0;
            z(:,i) = trace.y(start:end_) - 3.2;
