function z = read_wave3(FN1,FN2,num) % [ 자른 파형 리턴 ]
...
for i= st : st + num
    index = sprintf("%05d",i);
    trace = read_lecroywave("./wave/"+FN1+"--"+index+".trc",i);  %%%
    trigger = read_lecroywave("./wave/"+FN2+"--"+index+".trc",i);  %%%
    ...
    if count > 0
        if length(z(:,i-1)) > len 
            z(:,i) = vertcat(trace.y(start:end_) ,(zeros(length(z(:,i-1)) - len , 1)+3));
            
            %fprintf("%d",length(z(:,i-1)));
        else
            z(len,i) = 0;
            z(:,i) = trace.y(start:end_);
